...
async function reviewPaymentWithSupabase(input: ReviewInput): Promise<ReviewResult> {
  const supabase = getSupabaseServerClient();
  const { data: payment } = await supabase.from("payments").select("kind,order_id").eq("id", input.paymentId).maybeSingle();
  ...
  if (error) {
    ...
    if (message.includes("already") || message.includes("pending") || message.includes("invalid")) return { ok: false, status: 409, error: "That payment can no longer be reviewed." };
    return { ok: false, status: 500, error: "Unable to review payment." };
  }
  const row = data?.[0];
  if (!row || row.order_number !== input.orderNumber) return { ok: false, status: 404, error: "Payment not found." };

  if (payment?.order_id) {
    const { data: order } = await supabase.from("orders").select("id,email,customer_name").eq("id", payment.order_id).maybeSingle();
    if (order) {
      const template = input.action === "rejected" ? "payment_rejected" : payment.kind === "balance" ? "balance_approved" : "reservation_approved";
      await enqueueNotification({ orderId: order.id, recipient: order.email, template, payload: { orderNumber: input.orderNumber, customerName: order.customer_name, message: input.adminNote ?? undefined } });
    }
  }
...
export async function POST(request: Request, context: { params: Promise<{ orderNumber: string; paymentId: string }> }) {
